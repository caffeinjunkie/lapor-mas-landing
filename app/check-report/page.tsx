...
>([]);
  ...
  const {
    ...
    mutate: mutate,
  } = useSWR<Report>(["report", trackingId], () => trackingId && fetchTaskByTrackingId(trackingId) as any, {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const param = appendParams({ trackingId: dataFromEntries["tracking-id"] as string })
    router.replace(`${pathname}?${param}`)
  }

  const onImagePress = (images: { src: string; key: string }[], index: number) => {
    ...
    <div className="flex flex-col gap-8 w-full">
      <div className="flex flex-col lg:flex-row gap-8 w-full">
        <div className="flex flex-col w-full lg:w-96 rounded-lg gap-2">
          ...
          <Input type="text" label="Tracking ID" defaultValue={trackingId || ""} name="tracking-id" id="tracking-id" />
          <Button color="primary" type="submit" className="w-full">{t("check-report-cta-text")}</Button>
          </Form>
        </div>
        ...
        <div className="flex flex-col w-full">
          ...
          {(error && !isEmptyError(error as unknown) && trackingId) && (
            <div className="min-h-64 lg:h-full rounded-xl bg-gray-400/10 flex flex-col gap-3 justify-center items-center">
              ...
              <Button color="primary" className="w-fit" startContent={<ArrowPathIcon className="w-5 h-5" />} onPress={() => mutate()}>{t("check-report-try-again-text")}</Button>
            </div>
          )}
          {report && (
            <div className="flex flex-col gap-8 w-full">
