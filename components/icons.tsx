import * as React from "react";

import { IconSvgProps } from "@/types";

export const Logo: React.FC<IconSvgProps & { animated?: boolean }> = ({
  width = 152,
  height = 52,
  color = "#FF9F29",
  fill = "currentColor",
  animated = false,
  ...props
}) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={height}
    width={width}
    viewBox="0 0 500 500"
    {...props}
  >
    <g data-name="Layer 2">
      <path
        d="M467.2 633.55c1.72-1.69 3.74-3 6.08-3.92 2.33-.92 4.91-1.38 7.73-1.38s5.36.53 7.77 1.58c2.41 1.05 4.62 2.63 6.62 4.73l6.46-6.08c-2.51-2.97-5.59-5.22-9.23-6.73-3.64-1.51-7.67-2.27-12.08-2.27-4.16 0-7.98.68-11.46 2.04-3.49 1.36-6.54 3.3-9.16 5.81a26.687 26.687 0 0 0-6.08 8.77c-1.44 3.33-2.15 7.03-2.15 11.08s.72 7.75 2.15 11.08a27.03 27.03 0 0 0 6.04 8.77c2.59 2.51 5.63 4.45 9.12 5.81 3.49 1.36 7.31 2.04 11.47 2.04 4.46 0 8.51-.77 12.16-2.31 3.64-1.54 6.72-3.8 9.23-6.77l-6.46-6.08c-2 2.16-4.21 3.76-6.62 4.81-2.41 1.05-5 1.58-7.77 1.58s-5.4-.46-7.73-1.38c-2.33-.92-4.36-2.23-6.08-3.92-1.72-1.69-3.07-3.69-4.04-6-.98-2.31-1.46-4.85-1.46-7.62s.49-5.31 1.46-7.62c.97-2.31 2.32-4.31 4.04-6ZM536.88 650.86h26v-8.23h-26V628.7h29.31v-8.46h-39.32v53.86h40.4v-8.47h-30.39v-14.77zM631.75 656.77l-29.7-36.53h-8.23v53.86h9.92v-36.53l29.63 36.53h8.23v-53.86h-9.85v36.53zM664.61 628.7h17.85v45.4h10.01v-45.4h17.85v-8.46h-45.71v8.46zM743.25 650.86h26.01v-8.23h-26.01V628.7h29.32v-8.46h-39.32v53.86h40.4v-8.47h-30.4v-14.77zM846.21 674.1l-12.39-17.67c.27-.11.54-.21.8-.33 3.41-1.54 6.04-3.72 7.89-6.54 1.85-2.82 2.77-6.21 2.77-10.16s-.92-7.44-2.77-10.31-4.48-5.07-7.89-6.58c-3.41-1.51-7.5-2.27-12.27-2.27h-22.16v53.86h10v-15.7h12.16c.72 0 1.41-.02 2.1-.06l10.98 15.76h10.77Zm-14.35-42.59c2.23 1.87 3.35 4.5 3.35 7.89s-1.12 6.03-3.35 7.93c-2.23 1.9-5.55 2.85-9.96 2.85h-11.7v-21.47h11.7c4.41 0 7.73.94 9.96 2.81Z"
        className="cls-1"
        style={{
          fill: "#e79f29",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M502.31 431.56h-21.39v6.07h-34.29v21.39h34.29v116.79h21.39V459.02h34.29v-21.39h-34.29v-6.07zM542.84 458.82h10v96.97h-10v21.39h41.37v-21.39h-9.99v-96.97h9.99v-21.38h-41.37v21.38zM653.87 534.1l-35.64-99.48-20.21 7.2 3.02 8.37-.2 126.99h21.39v-67.91l22.03 61.05h19.4l22.03-61.05v67.91h21.39V450.19l3.02-8.37-20.21-7.2-36.02 99.48zM820.62 508.49l48.79-60.21-16.51-13.49-46.87 57.79V439.2h-.01v-1.77h-42.64v.01H723.9v21.38h9.99v96.97h-9.99v21.39h41.37v-21.39h-9.99v-96.97h9.99v-.01h19.38v60.05l-12.06 14.75 12.06 9.77v35.17h21.38v-52.2l.82-.94 42.55 53.14h63.43v-21.39h-53.2l-39.01-48.67z"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M938.85 548.77h1.8v-21.39h-7.86l-25.78-90.92-20.67 5.81 2.09 7.43-28.31 99.07h56.61l8.93 31.36 20.48-5.82-7.3-25.54Zm-50.36-21.39 11.1-38.67 11.1 38.67H888.5ZM733.87 405.04h21.37v21.37h-21.37zM552.91 405.04h21.37v21.37h-21.37z"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="m234.81 664.9-.07.07 29.15 29.95c2.21 2.27 5.17 3.54 8.34 3.56 3.16 0 6.15-1.2 8.39-3.44l29.95-29.91-.03-.03h.04l.04-39.01.02-15.66-15.66-.02-31.26-.03-.05 43.71c0 4.32 3.5 7.83 7.82 7.84 4.32 0 7.83-3.5 7.84-7.82l.03-28.05 15.6.02-.03 29.47-22.56 22.53-22-22.6.15-139.13-15.66-.02-.16 148.58h.1Z"
        className="cls-2"
        data-name="&amp;lt;Radial Repeat&amp;gt;"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="m161.9 516.08-.07-.07-29.95 29.15a11.664 11.664 0 0 0-3.56 8.34c0 3.16 1.2 6.15 3.44 8.39l29.91 29.95.03-.03v.04l39.01.04 15.66.02.02-15.66.03-31.26-43.71-.05c-4.32 0-7.83 3.5-7.84 7.82 0 4.32 3.5 7.83 7.82 7.84l28.05.03-.02 15.6-29.47-.03-22.53-22.56 22.6-22 139.13.15.02-15.66-148.58-.16v.1Z"
        className="cls-2"
        data-name="&amp;lt;Radial Repeat&amp;gt;"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="m310.72 443.17.07-.07-29.15-29.95a11.664 11.664 0 0 0-8.34-3.56c-3.16 0-6.15 1.2-8.39 3.44l-29.95 29.91.03.03h-.04l-.04 39.01-.02 15.66 15.66.02 31.26.03.05-43.71c0-4.32-3.5-7.83-7.82-7.84-4.32 0-7.83 3.5-7.84 7.82l-.03 28.05-15.6-.02.03-29.47 22.56-22.53 22 22.6-.15 139.13 15.66.02.16-148.58h-.1Z"
        className="cls-2"
        data-name="&amp;lt;Radial Repeat&amp;gt;"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="m383.63 591.99.07.07 29.95-29.15c2.27-2.21 3.54-5.17 3.56-8.34 0-3.16-1.2-6.15-3.44-8.39l-29.91-29.95-.03.03v-.04l-39.01-.04-15.66-.02-.02 15.66-.03 31.26 43.71.05c4.32 0 7.83-3.5 7.84-7.82 0-4.32-3.5-7.83-7.82-7.84l-28.05-.03.02-15.6 29.47.03 22.53 22.56-22.6 22-139.13-.15-.02 15.66 148.58.16v-.1Z"
        className="cls-2"
        data-name="&amp;lt;Radial Repeat&amp;gt;"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M193.76 636.14s-8.27-38.84 30.81-29.91"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M193.79 636.13s38.58 9.4 30.8-29.93M324.76 505.14s-8.27-38.84 30.81-29.91"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M324.78 505.13s38.58 9.4 30.8-29.93"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M260.97 542.24h23.59v23.59h-23.59z"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="rotate(-90 -21.73 546.77)"
      />
      <path
        d="M352.98 635.12s-38.84 8.27-29.91-30.81"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M352.97 635.09s9.4-38.58-29.93-30.8M221.98 504.12s-38.84 8.27-29.91-30.81"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
      <path
        d="M221.97 504.09s9.4-38.58-29.93-30.8"
        className="cls-2"
        style={{
          fill: "#700002",
        }}
        transform="translate(-287.23 -301.76)"
      />
    </g>
  </svg>
);

export const AchievementIcon = ({
  size = 20,
  height,
  width,
  ...props
}: IconSvgProps) => (
  <svg
    width={width || size}
    height={height || size}
    viewBox="0 0 32 32"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        d="M16 2C13.0826 2 10.2847 3.15893 8.22184 5.22183C6.15894 7.28473 5.00001 10.0826 5.00001 13V29C4.9992 29.1519 5.03299 29.302 5.09883 29.4389C5.16468 29.5757 5.26084 29.6958 5.38001 29.79C5.5002 29.8835 5.64015 29.9483 5.78919 29.9795C5.93823 30.0107 6.09242 30.0075 6.24001 29.97L15 27.77V24C15 23.7348 15.1054 23.4804 15.2929 23.2929C15.4804 23.1054 15.7348 23 16 23C16.2652 23 16.5196 23.1054 16.7071 23.2929C16.8947 23.4804 17 23.7348 17 24V27.81L25.76 30C25.8397 30.0096 25.9203 30.0096 26 30C26.2652 30 26.5196 29.8946 26.7071 29.7071C26.8947 29.5196 27 29.2652 27 29V13C27 10.0826 25.8411 7.28473 23.7782 5.22183C21.7153 3.15893 18.9174 2 16 2Z"
        fill="#FBC02D"
      />
      <path
        d="M5.00001 13V29C4.9992 29.1519 5.03299 29.302 5.09883 29.4389C5.16468 29.5757 5.26084 29.6958 5.38001 29.79C5.5002 29.8835 5.64015 29.9483 5.78919 29.9795C5.93823 30.0107 6.09242 30.0075 6.24001 29.97L15 27.8V24C14.9994 23.7465 15.0951 23.5022 15.2677 23.3165C15.4404 23.1309 15.6771 23.0177 15.93 23V2C13.0248 2.01849 10.2448 3.18557 8.19705 5.24647C6.14927 7.30737 4.99996 10.0947 5.00001 13Z"
        fill="#FDD835"
      />
      <path
        d="M22.87 10.92C22.812 10.7422 22.7054 10.5843 22.5623 10.4641C22.4191 10.3439 22.2451 10.2663 22.06 10.24L18.46 9.70997L16.85 6.44997C16.7681 6.2809 16.6403 6.13832 16.4811 6.03855C16.3219 5.93878 16.1379 5.88586 15.95 5.88586C15.7621 5.88586 15.5781 5.93878 15.4189 6.03855C15.2597 6.13832 15.1319 6.2809 15.05 6.44997L13.44 9.70997L9.84 10.24C9.65686 10.2682 9.48516 10.3468 9.34409 10.467C9.20301 10.5871 9.09809 10.7441 9.04105 10.9205C8.984 11.0968 8.97708 11.2855 9.02104 11.4655C9.065 11.6455 9.15812 11.8098 9.29 11.94L11.89 14.48L11.28 18.07C11.2432 18.2565 11.2604 18.4496 11.3295 18.6267C11.3985 18.8038 11.5167 18.9576 11.67 19.07C11.8213 19.1816 12.001 19.2483 12.1885 19.2625C12.376 19.2766 12.5637 19.2376 12.73 19.15L16 17.43L19.22 19.13C19.3668 19.2004 19.5272 19.2379 19.69 19.24C19.8984 19.2386 20.1012 19.1722 20.27 19.05C20.4252 18.9388 20.5453 18.7856 20.6161 18.6084C20.687 18.4312 20.7057 18.2375 20.67 18.05L20 14.48L22.6 11.94C22.7356 11.812 22.8328 11.6487 22.8805 11.4685C22.9282 11.2882 22.9246 11.0983 22.87 10.92Z"
        fill="#C2185B"
      />
      <path
        d="M15.05 6.44995L13.44 9.70995L9.84002 10.23C9.65375 10.2571 9.47892 10.3362 9.33562 10.4583C9.19233 10.5803 9.0864 10.7404 9.03002 10.92C8.9782 11.0945 8.97504 11.2799 9.02086 11.4561C9.06668 11.6324 9.15974 11.7927 9.29002 11.92L11.89 14.46L11.28 18.05C11.2495 18.2334 11.2708 18.4217 11.3415 18.5938C11.4121 18.7658 11.5293 18.9147 11.68 19.0238C11.8306 19.1329 12.0087 19.1978 12.1942 19.2113C12.3797 19.2247 12.5652 19.1862 12.73 19.1L16 17.43V5.93995C15.8095 5.92531 15.6188 5.96556 15.4504 6.05595C15.282 6.14633 15.1431 6.28306 15.05 6.44995Z"
        fill="#EC407A"
      />
      <path
        d="M16 2C13.0826 2 10.2847 3.15893 8.22184 5.22183C6.15894 7.28473 5.00001 10.0826 5.00001 13V29C4.9992 29.1519 5.03299 29.302 5.09883 29.4389C5.16468 29.5757 5.26084 29.6958 5.38001 29.79C5.5002 29.8835 5.64015 29.9483 5.78919 29.9795C5.93823 30.0107 6.09242 30.0075 6.24001 29.97L16 27.53L25.76 30C25.8397 30.0096 25.9203 30.0096 26 30C26.2652 30 26.5196 29.8946 26.7071 29.7071C26.8947 29.5196 27 29.2652 27 29V13C27 10.0826 25.8411 7.28473 23.7782 5.22183C21.7153 3.15893 18.9174 2 16 2ZM16 4C17.78 4 19.5201 4.52784 21.0001 5.51677C22.4802 6.50571 23.6337 7.91131 24.3149 9.55585C24.9961 11.2004 25.1744 13.01 24.8271 14.7558C24.4798 16.5016 23.6226 18.1053 22.364 19.364C21.1053 20.6226 19.5017 21.4798 17.7558 21.8271C16.01 22.1743 14.2004 21.9961 12.5559 21.3149C10.9113 20.6337 9.50572 19.4802 8.51679 18.0001C7.52786 16.5201 7.00001 14.78 7.00001 13C7.00001 10.6131 7.94823 8.32387 9.63605 6.63604C11.3239 4.94821 13.6131 4 16 4ZM7.00001 27.71V19.28C7.91568 20.5897 9.10571 21.6841 10.4873 22.4871C11.8689 23.2902 13.4088 23.7825 15 23.93V25.71L7.00001 27.71ZM25 27.71L17 25.71V23.91C18.5912 23.7625 20.1312 23.2702 21.5127 22.4671C22.8943 21.6641 24.0843 20.5697 25 19.26V27.71Z"
        fill="#263238"
      />
    </g>
  </svg>
);

export const CameraIcon: React.FC<IconSvgProps> = ({
  fill = "currentColor",
  size,
  height,
  width,
  ...props
}) => (
  <svg
    fill="none"
    height={size || height || 24}
    viewBox="0 0 24 24"
    width={size || width || 24}
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      clipRule="evenodd"
      d="M17.44 6.236c.04.07.11.12.2.12 2.4 0 4.36 1.958 4.36 4.355v5.934A4.368 4.368 0 0117.64 21H6.36A4.361 4.361 0 012 16.645V10.71a4.361 4.361 0 014.36-4.355c.08 0 .16-.04.19-.12l.06-.12.106-.222a97.79 97.79 0 01.714-1.486C7.89 3.51 8.67 3.01 9.64 3h4.71c.97.01 1.76.51 2.22 1.408.157.315.397.822.629 1.31l.141.299.1.22zm-.73 3.836c0 .5.4.9.9.9s.91-.4.91-.9-.41-.909-.91-.909-.9.41-.9.91zm-6.44 1.548c.47-.47 1.08-.719 1.73-.719.65 0 1.26.25 1.72.71.46.459.71 1.068.71 1.717A2.438 2.438 0 0112 15.756c-.65 0-1.26-.25-1.72-.71a2.408 2.408 0 01-.71-1.717v-.01c-.01-.63.24-1.24.7-1.699zm4.5 4.485a3.91 3.91 0 01-2.77 1.15 3.921 3.921 0 01-3.93-3.926 3.865 3.865 0 011.14-2.767A3.921 3.921 0 0112 9.402c1.05 0 2.04.41 2.78 1.15.74.749 1.15 1.738 1.15 2.777a3.958 3.958 0 01-1.16 2.776z"
      fill={fill}
      fillRule="evenodd"
    />
  </svg>
);

export const BrokenImageIcon = ({
  size = 24,
  width,
  height,
  color = "currentColor",
  ...props
}: IconSvgProps) => (
  <svg
    fill={color}
    height={size || height}
    width={size || width}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path d="M21.71,14.54,19.21,12a1,1,0,0,0-1.42,0L15,14.84,12.21,12a1,1,0,0,0-1.42,0L8.5,14.34,6.21,12a1,1,0,0,0-1.42,0l-2.5,2.5a1,1,0,0,0-.21.33,1,1,0,0,0-.08.38V19a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V15.25a1,1,0,0,0-.08-.38A1,1,0,0,0,21.71,14.54ZM20,19a1,1,0,0,1-1,1H5a1,1,0,0,1-1-1V15.66l1.5-1.5,2.29,2.3a1,1,0,0,0,1.42,0l2.29-2.3L14.29,17a1,1,0,0,0,1.42,0l2.79-2.8,1.5,1.5ZM19,2H5A3,3,0,0,0,2,5v5.26a1.17,1.17,0,0,0,0,.27s0,.07,0,.1a1,1,0,0,0,1.66.31L5.5,9.16l2.29,2.3a1,1,0,0,0,1.42,0l2.29-2.3L14.29,12a1,1,0,0,0,1.42,0l2.79-2.8,1.77,1.78a1,1,0,0,0,1.66-.31.28.28,0,0,0,0-.09.88.88,0,0,0,.06-.28V5A3,3,0,0,0,19,2Zm1,5.84L19.21,7a1,1,0,0,0-1.42,0L15,9.84,12.21,7a1,1,0,0,0-1.42,0L8.5,9.34,6.21,7A1,1,0,0,0,4.79,7L4,7.84V5A1,1,0,0,1,5,4H19a1,1,0,0,1,1,1Z" />
    </g>
  </svg>
);

export const ImageIcon = ({
  size = 24,
  width,
  height,
  color = "currentColor",
  stroke = color,
  ...props
}: IconSvgProps) => (
  <svg
    height={size || height}
    width={size || width}
    fill={color}
    stroke={stroke}
    viewBox="0 0 512 512"
    strokeWidth={0.5}
    strokeLinecap="round"
    strokeLinejoin="round"
    {...props}
  >
    <path
      d="M378.413,0H208.297h-13.168l-9.314,9.314L57.02,138.102l-9.314,9.314v13.176v265.514c0,47.36,38.527,85.895,85.895,85.895h244.812c47.353,0,85.881-38.535,85.881-85.895V85.896C464.294,38.528,425.766,0,378.413,0z M432.497,426.105c0,29.877-24.214,54.091-54.084,54.091H133.601c-29.87,0-54.084-24.214-54.084-54.091V160.591h83.717c24.884,0,45.063-20.178,45.063-45.07V31.804h170.116c29.87,0,54.084,24.214,54.084,54.092V426.105z"
      fill={color}
    />
    <path
      d="M162.94,251.968c-5.851,0-10.054,4.21-10.054,10.592v72.804c0,6.388,4.203,10.599,10.054,10.599c5.698,0,9.915-4.21,9.915-10.599V262.56C172.855,256.178,168.638,251.968,162.94,251.968z"
      fill={color}
    />
    <path
      d="M265.621,251.968c-5.977,0-9.244,3.261-12.219,10.326l-19.299,44.547h-0.545l-19.69-44.547c-3.114-7.066-6.382-10.326-12.358-10.326c-6.647,0-11.004,4.622-11.004,11.954v72.398c0,6.109,3.812,9.643,9.245,9.643c5.153,0,8.965-3.534,8.965-9.643v-44.554h0.67l14.398,33.138c2.848,6.522,5.167,8.428,9.775,8.428c4.622,0,6.926-1.906,9.789-8.428l14.258-33.138h0.684v44.554c0,6.109,3.658,9.643,9.091,9.643c5.432,0,9.105-3.534,9.105-9.643v-72.398C276.486,256.59,272.269,251.968,265.621,251.968z"
      fill={color}
    />
    <path
      d="M356.363,293.806h-19.02c-5.153,0-8.42,3.121-8.42,7.876c0,4.755,3.268,7.876,8.42,7.876h6.256c0.545,0,0.81,0.272,0.81,0.816c0,3.533-0.266,6.654-1.089,9.098c-1.9,5.844-7.737,9.51-14.803,9.51c-8.015,0-13.043-3.938-15.068-10.187c-1.089-3.393-1.494-7.876-1.494-19.83c0-11.953,0.406-16.296,1.494-19.696c2.025-6.382,6.927-10.32,14.802-10.32c5.977,0,10.459,1.899,13.993,6.786c2.709,3.805,5.432,4.895,8.825,4.895c5.028,0,9.091-3.666,9.091-8.965c0-2.171-0.67-4.078-1.76-5.977c-4.888-8.287-15.207-14.397-30.149-14.397c-16.436,0-29.199,7.471-33.962,22.412c-2.038,6.515-2.583,11.682-2.583,25.262c0,13.581,0.545,18.74,2.583,25.262c4.762,14.942,17.526,22.413,33.962,22.413c16.436,0,28.921-8.288,33.683-23.09c1.634-5.16,2.304-12.77,2.304-20.919v-0.95C364.238,296.654,361.39,293.806,356.363,293.806z"
      fill={color}
    />
  </svg>
);

export const MoonFilledIcon = ({
  size = 24,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    aria-hidden="true"
    focusable="false"
    height={size || height}
    role="presentation"
    viewBox="0 0 24 24"
    width={size || width}
    {...props}
  >
    <path
      d="M21.53 15.93c-.16-.27-.61-.69-1.73-.49a8.46 8.46 0 01-1.88.13 8.409 8.409 0 01-5.91-2.82 8.068 8.068 0 01-1.44-8.66c.44-1.01.13-1.54-.09-1.76s-.77-.55-1.83-.11a10.318 10.318 0 00-6.32 10.21 10.475 10.475 0 007.04 8.99 10 10 0 002.89.55c.16.01.32.02.48.02a10.5 10.5 0 008.47-4.27c.67-.93.49-1.519.32-1.79z"
      fill="currentColor"
    />
  </svg>
);

export const SunFilledIcon = ({
  size = 24,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    aria-hidden="true"
    focusable="false"
    height={size || height}
    role="presentation"
    viewBox="0 0 24 24"
    width={size || width}
    {...props}
  >
    <g fill="currentColor">
      <path d="M19 12a7 7 0 11-7-7 7 7 0 017 7z" />
      <path d="M12 22.96a.969.969 0 01-1-.96v-.08a1 1 0 012 0 1.038 1.038 0 01-1 1.04zm7.14-2.82a1.024 1.024 0 01-.71-.29l-.13-.13a1 1 0 011.41-1.41l.13.13a1 1 0 010 1.41.984.984 0 01-.7.29zm-14.28 0a1.024 1.024 0 01-.71-.29 1 1 0 010-1.41l.13-.13a1 1 0 011.41 1.41l-.13.13a1 1 0 01-.7.29zM22 13h-.08a1 1 0 010-2 1.038 1.038 0 011.04 1 .969.969 0 01-.96 1zM2.08 13H2a1 1 0 010-2 1.038 1.038 0 011.04 1 .969.969 0 01-.96 1zm16.93-7.01a1.024 1.024 0 01-.71-.29 1 1 0 010-1.41l.13-.13a1 1 0 011.41 1.41l-.13.13a.984.984 0 01-.7.29zm-14.02 0a1.024 1.024 0 01-.71-.29l-.13-.14a1 1 0 011.41-1.41l.13.13a1 1 0 010 1.41.97.97 0 01-.7.3zM12 3.04a.969.969 0 01-1-.96V2a1 1 0 012 0 1.038 1.038 0 01-1 1.04z" />
    </g>
  </svg>
);

export const RefreshIcon = ({
  size = 18,
  fill = "currentColor",
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    fill="none"
    height={height || size}
    viewBox="0 0 1024 1024"
    width={width || size}
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        clipRule="evenodd"
        d="M289.088 296.704h92.992a32 32 0 0 1 0 64H232.96a32 32 0 0 1-32-32V179.712a32 32 0 0 1 64 0v50.56a384 384 0 0 1 643.84 282.88 384 384 0 0 1-383.936 384 384 384 0 0 1-384-384h64a320 320 0 1 0 640 0 320 320 0 0 0-555.712-216.448z"
        fill={fill}
        fillRule="evenodd"
      />
    </g>
  </svg>
);

export const MapIcon = ({
  size = 18,
  fill = "currentColor",
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    fill="none"
    height={height || size}
    viewBox="0 0 24 24"
    width={width || size}
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        d="M12.5 7.04148C12.3374 7.0142 12.1704 7 12 7C10.3431 7 9 8.34315 9 10C9 11.6569 10.3431 13 12 13C13.6569 13 15 11.6569 15 10C15 9.82964 14.9858 9.6626 14.9585 9.5"
        stroke={fill}
        strokeLinecap="round"
        strokeWidth="1.5"
      />
      <path
        d="M5 15.2161C4.35254 13.5622 4 11.8013 4 10.1433C4 5.64588 7.58172 2 12 2C16.4183 2 20 5.64588 20 10.1433C20 14.6055 17.4467 19.8124 13.4629 21.6744C12.5343 22.1085 11.4657 22.1085 10.5371 21.6744C9.26474 21.0797 8.13831 20.1439 7.19438 19"
        stroke={fill}
        strokeLinecap="round"
        strokeWidth="1.5"
      />
    </g>
  </svg>
);

export const SecurityIcon = ({
  size = 28,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    fill="none"
    height={height || size}
    width={width || size}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        d="M18.3301 5.67L6.59009 17.41C6.15009 17.85 5.41008 17.79 5.05008 17.27C3.81008 15.46 3.08008 13.32 3.08008 11.12V6.73C3.08008 5.91 3.70008 4.98 4.46008 4.67L10.0301 2.39C11.2901 1.87 12.6901 1.87 13.9501 2.39L17.9901 4.03999C18.6601 4.30999 18.8301 5.17 18.3301 5.67Z"
        fill="#FBA518"
      />
      <path
        d="M19.27 7.04012C19.92 6.49012 20.91 6.96011 20.91 7.81011V11.1201C20.91 16.0101 17.36 20.5901 12.51 21.9301C12.18 22.0201 11.82 22.0201 11.48 21.9301C10.06 21.5301 8.74 20.8601 7.61 19.9801C7.13 19.6101 7.08001 18.9101 7.50001 18.4801C9.68001 16.2501 16.06 9.75012 19.27 7.04012Z"
        fill="#3674B5"
      />
    </g>
  </svg>
);

export const PublicFacilityIcon = ({
  size = 28,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    height={height || size}
    width={width || size}
    version="1.1"
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512 512"
    fill="#000000"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        fill="#A9D5BB"
        d="M439.242,482.358H83.537c-23.812,0-43.116-19.304-43.116-43.116V72.758 c0-23.812,19.304-43.116,43.116-43.116h355.705c23.812,0,43.116,19.304,43.116,43.116v366.484 C482.358,463.054,463.054,482.358,439.242,482.358z"
      />
      <path
        fill="#C5EAD4"
        d="M385.347,51.2L385.347,51.2c0,11.906-9.651,21.558-21.558,21.558H245.221 c-11.906,0-21.558-9.651-21.558-21.558h-64.674c-11.906,0-21.558-9.651-21.558-21.558l0,0c0-11.906,9.651-21.558,21.558-21.558H256 c11.906,0,21.558,9.651,21.558,21.558l0,0h86.232C375.696,29.642,385.347,39.294,385.347,51.2z"
      />
      <path
        fill="#74843A"
        d="M493.137,315.284v188.632h-86.232h-32.337h-53.895h-32.337h-64.674h-21.558h-64.674h-32.337H18.863 V282.947c0-32.742,26.542-59.284,59.284-59.284l0,0c9.708,0,18.86,2.348,26.947,6.484v-33.431c0-32.742,26.542-59.284,59.284-59.284 l0,0c32.742,0,59.284,26.542,59.284,59.284v40.499c10.25-8.466,23.395-13.551,37.726-13.551l0,0c10.951,0,21.201,2.981,30.004,8.16 c7.859-23.529,30.055-40.496,56.227-40.496l0,0c32.742,0,59.284,26.542,59.284,59.284v11.873c8.087-4.136,17.24-6.484,26.947-6.484 l0,0C466.595,256,493.137,282.542,493.137,315.284z"
      />
      <path
        fill="#9FBA41"
        d="M164.052,67.152l23.263,271.398c2.158,25.173-17.694,46.798-42.958,46.798H55.053 c-25.265,0-45.116-21.625-42.958-46.798L35.358,67.152C38.222,33.743,66.175,8.084,99.705,8.084l0,0 C133.235,8.084,161.188,33.743,164.052,67.152z"
      />
      <path
        fill="#FFED8F"
        d="M503.916,191.326L503.916,191.326c0,11.906-9.651,21.558-21.558,21.558 c-11.906,0-21.558-9.651-21.558-21.558l0,0c0-11.906,9.651-21.558,21.558-21.558C494.264,169.768,503.916,179.42,503.916,191.326z M374.568,169.768c-11.906,0-21.558,9.651-21.558,21.558l0,0c0,11.906,9.651,21.558,21.558,21.558 c11.906,0,21.558-9.651,21.558-21.558l0,0C396.126,179.42,386.475,169.768,374.568,169.768z"
      />
      <path
        fill="#DDB961"
        d="M132.042,406.905v-43.116c0-5.953,4.826-10.779,10.779-10.779h226.358 c5.953,0,10.779,4.826,10.779,10.779v43.116c0,5.953-4.826,10.779-10.779,10.779H142.821 C136.868,417.684,132.042,412.858,132.042,406.905z"
      />
      <path d="M215.579,237.214v-40.499c0-15.813-7.14-30.495-19.59-40.28c-3.51-2.759-4.119-7.841-1.36-11.351 c2.758-3.512,7.842-4.119,11.351-1.36c16.375,12.87,25.767,32.185,25.767,52.991v25.739c9.169-4.487,19.319-6.876,29.642-6.876 c8.901,0,17.693,1.767,25.852,5.159c6.994-14.119,18.806-25.484,33.408-31.874c4.091-1.788,8.857,0.074,10.647,4.165 c1.79,4.09-0.074,8.857-4.165,10.647c-13.258,5.802-23.489,16.995-28.071,30.707c-0.773,2.313-2.548,4.154-4.832,5.009 c-2.285,0.857-4.832,0.635-6.934-0.603c-7.834-4.606-16.794-7.042-25.906-7.042c-11.873,0-23.443,4.155-32.578,11.701 c-1.478,1.22-3.305,1.851-5.149,1.851c-1.168,0-2.344-0.253-3.444-0.771C217.387,243.194,215.579,240.346,215.579,237.214z M303.068,37.726h60.721c7.43,0,13.474,6.044,13.474,13.474s-6.044,13.474-13.474,13.474H245.221 c-7.43,0-13.474-6.044-13.474-13.474c0-4.466-3.62-8.084-8.084-8.084h-40.96c-4.465,0-8.084,3.618-8.084,8.084 s3.62,8.084,8.084,8.084H216.7c3.527,12.428,14.978,21.558,28.521,21.558h118.568c16.345,0,29.642-13.298,29.642-29.642 s-13.297-29.642-29.642-29.642h-60.721c-4.465,0-8.084,3.618-8.084,8.084S298.604,37.726,303.068,37.726z M512,503.916 c0,4.466-3.62,8.084-8.084,8.084H300.553c-4.465,0-8.084-3.618-8.084-8.084s3.62-8.084,8.084-8.084h33.595v-37.726H177.853v37.726 h90.004c4.465,0,8.084,3.618,8.084,8.084s-3.62,8.084-8.084,8.084H8.084C3.62,512,0,508.382,0,503.916s3.62-8.084,8.084-8.084h2.695 v-88.926c0-4.466,3.62-8.084,8.084-8.084s8.084,3.618,8.084,8.084v88.926h64.674v-102.4H55.053c-14.3,0-28.048-6.042-37.718-16.577 C7.665,366.321,2.82,352.107,4.041,337.859L27.303,66.461c1.559-18.18,9.82-34.987,23.262-47.326S81.459,0,99.705,0 s35.699,6.796,49.14,19.135c13.441,12.339,21.703,29.146,23.262,47.326l21.915,255.674c0.382,4.448-2.916,8.364-7.364,8.745 c-4.447,0.376-8.364-2.916-8.745-7.364L155.998,67.842c-2.526-29.459-26.726-51.673-56.293-51.673S45.938,38.383,43.412,67.842 L20.15,339.24c-0.848,9.892,2.382,19.368,9.096,26.681c6.714,7.314,15.88,11.342,25.807,11.342h36.568V115.874 c0-4.466,3.62-8.084,8.084-8.084s8.084,3.618,8.084,8.084v379.958h53.895v-37.726h-29.642c-4.465,0-8.084-3.618-8.084-8.084 s3.62-8.084,8.084-8.084h29.642v-16.168h-18.863c-10.401,0-18.863-8.463-18.863-18.863v-43.116c0-10.401,8.463-18.863,18.863-18.863 h226.358c10.401,0,18.863,8.463,18.863,18.863v43.116c0,10.401-8.463,18.863-18.863,18.863h-18.863v16.168h29.642 c4.465,0,8.084,3.618,8.084,8.084s-3.62,8.084-8.084,8.084h-29.642v37.726h70.523c-0.298-0.843-0.46-1.749-0.46-2.695V265.882 c-3.371,0.921-6.645,2.189-9.793,3.8c-2.506,1.279-5.498,1.167-7.9-0.302c-2.402-1.468-3.865-4.081-3.865-6.895v-11.874 c0-6.456-1.184-12.746-3.518-18.696c-1.631-4.155,0.416-8.847,4.572-10.478c4.156-1.634,8.848,0.416,10.478,4.572 c3.075,7.836,4.635,16.11,4.636,24.593c1.776-0.518,3.573-0.964,5.389-1.333V134.737h-24.253c-7.43,0-13.474,6.044-13.474,13.474 v14.594c12.428,3.527,21.558,14.978,21.558,28.522c0,16.344-13.297,29.642-29.642,29.642c-16.345,0-29.642-13.298-29.642-29.642 c0-13.543,9.13-24.994,21.558-28.522v-14.594c0-16.344,13.297-29.642,29.642-29.642H460.8c16.345,0,29.642,13.298,29.642,29.642 v14.594C502.87,166.331,512,177.783,512,191.326c0,16.344-13.297,29.642-29.642,29.642c-16.345,0-29.642-13.298-29.642-29.642 c0-13.543,9.13-24.994,21.558-28.522v-14.594c0-7.43-6.044-13.474-13.474-13.474h-24.253v358.4c0,0.945-0.162,1.852-0.46,2.695 h48.966V315.284c0-21.085-13.226-40.309-32.91-47.839c-4.17-1.594-6.257-6.268-4.662-10.439c1.594-4.17,6.269-6.255,10.439-4.662 c25.9,9.908,43.301,35.202,43.301,62.94v180.547h2.695C508.38,495.832,512,499.45,512,503.916z M468.884,191.326 c0,7.43,6.044,13.474,13.474,13.474s13.474-6.044,13.474-13.474s-6.044-13.474-13.474-13.474S468.884,183.896,468.884,191.326z M374.568,177.853c-7.43,0-13.474,6.044-13.474,13.474s6.044,13.474,13.474,13.474c7.43,0,13.474-6.044,13.474-13.474 S381.998,177.853,374.568,177.853z M334.147,425.768H177.853v16.168h156.295V425.768z M342.232,409.6h26.947 c1.485,0,2.695-1.208,2.695-2.695v-43.116c0-1.486-1.209-2.695-2.695-2.695H142.821c-1.485,0-2.695,1.208-2.695,2.695v43.116 c0,1.486,1.209,2.695,2.695,2.695H342.232z M169.229,16.168H256c7.43,0,13.474,6.044,13.474,13.474c0,4.466,3.62,8.084,8.084,8.084 c4.465,0,8.084-3.618,8.084-8.084C285.642,13.298,272.345,0,256,0h-86.771c-4.465,0-8.084,3.618-8.084,8.084 S164.765,16.168,169.229,16.168z" />
    </g>
  </svg>
);

export const ConeIcon = ({
  size = 28,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    width={width || size}
    height={width || size}
    viewBox="0 0 48 48"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <rect width="48" height="48" fill="white" fillOpacity="0.01" />
      <path
        d="M27 6H21L19.75 10.5L18.5 15L16 24L13.5 33L12.25 37.5L11 42H37L35.5 36.6L32 24L29.5 15L28.25 10.5L27 6Z"
        fill="#FFAF00"
      />
      <path
        d="M37 42H11M37 42H6H11M37 42H42M37 42L35.5 36.6L32 24L29.5 15L28.25 10.5L27 6H21L19.75 10.5L18.5 15L16 24L13.5 33L12.25 37.5L11 42"
        stroke="#FF9D23"
        strokeWidth="4"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M13.5 33H34.5"
        stroke="white"
        strokeWidth="4"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M32 24H16"
        stroke="white"
        strokeWidth="4"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M29.5 15H18.5"
        stroke="white"
        strokeWidth="4"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M28.25 10.5L35.5 36.6"
        stroke="#FF9D23"
        strokeWidth="4"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M12.25 37.5L19.75 10.5"
        stroke="#FF9D23"
        strokeWidth="4"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </g>
  </svg>
);

export const BuildingIcon = ({
  size = 24,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    version="1.0"
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    width={width || size}
    height={width || size}
    viewBox="0 0 64 64"
    fill="#000000"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <g>
        <g>
          <path
            fill="#B4CCB9"
            d="M2,15v46c0,1.104,0.896,2,2,2h12V13H4C2.896,13,2,13.896,2,15z"
          />
          <path
            fill="#B4CCB9"
            d="M44,3H20c-1.104,0-2,0.896-2,2v58l0.001,0.002H27V54c0-0.553,0.447-1,1-1h8c0.553,0,1,0.447,1,1v9.002 h8.999L46,63V5C46,3.896,45.104,3,44,3z"
          />
          <path
            fill="#B4CCB9"
            d="M60,23H48v40h12c1.104,0,2-0.896,2-2V25C62,23.896,61.104,23,60,23z"
          />
        </g>
        <path
          fill="#394240"
          d="M60,21H48V5c0-2.211-1.789-4-4-4H20c-2.211,0-4,1.789-4,4v6H4c-2.211,0-4,1.789-4,4v46c0,2.211,1.789,4,4,4 h56c2.211,0,4-1.789,4-4V25C64,22.789,62.211,21,60,21z M16,63H4c-1.104,0-2-0.896-2-2V15c0-1.104,0.896-2,2-2h12V63z M35,63.002 h-6V55h6V63.002z M46,63l-0.001,0.002H37V54c0-0.553-0.447-1-1-1h-8c-0.553,0-1,0.447-1,1v9.002h-8.999L18,63V5 c0-1.104,0.896-2,2-2h24c1.104,0,2,0.896,2,2V63z M62,61c0,1.104-0.896,2-2,2H48V23h12c1.104,0,2,0.896,2,2V61z"
        />
        <path
          fill="#394240"
          d="M7,25h4c0.553,0,1-0.447,1-1v-4c0-0.553-0.447-1-1-1H7c-0.553,0-1,0.447-1,1v4C6,24.553,6.447,25,7,25z M8,21h2v2H8V21z"
        />
        <path
          fill="#394240"
          d="M7,35h4c0.553,0,1-0.447,1-1v-4c0-0.553-0.447-1-1-1H7c-0.553,0-1,0.447-1,1v4C6,34.553,6.447,35,7,35z M8,31h2v2H8V31z"
        />
        <path
          fill="#394240"
          d="M7,45h4c0.553,0,1-0.447,1-1v-4c0-0.553-0.447-1-1-1H7c-0.553,0-1,0.447-1,1v4C6,44.553,6.447,45,7,45z M8,41h2v2H8V41z"
        />
        <path
          fill="#394240"
          d="M29,19h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C30,19.447,29.553,19,29,19 z M28,23h-2v-2h2V23z"
        />
        <path
          fill="#394240"
          d="M29,29h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C30,29.447,29.553,29,29,29 z M28,33h-2v-2h2V33z"
        />
        <path
          fill="#394240"
          d="M29,39h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C30,39.447,29.553,39,29,39 z M28,43h-2v-2h2V43z"
        />
        <path
          fill="#394240"
          d="M39,19h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C40,19.447,39.553,19,39,19 z M38,23h-2v-2h2V23z"
        />
        <path
          fill="#394240"
          d="M29,9h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C30,9.447,29.553,9,29,9z M28,13h-2v-2h2V13z"
        />
        <path
          fill="#394240"
          d="M39,9h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C40,9.447,39.553,9,39,9z M38,13h-2v-2h2V13z"
        />
        <path
          fill="#394240"
          d="M39,29h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C40,29.447,39.553,29,39,29 z M38,33h-2v-2h2V33z"
        />
        <path
          fill="#394240"
          d="M39,39h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C40,39.447,39.553,39,39,39 z M38,43h-2v-2h2V43z"
        />
        <path
          fill="#394240"
          d="M57,29h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C58,29.447,57.553,29,57,29 z M56,33h-2v-2h2V33z"
        />
        <path
          fill="#394240"
          d="M57,39h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C58,39.447,57.553,39,57,39 z M56,43h-2v-2h2V43z"
        />
        <path
          fill="#394240"
          d="M57,49h-4c-0.553,0-1,0.447-1,1v4c0,0.553,0.447,1,1,1h4c0.553,0,1-0.447,1-1v-4C58,49.447,57.553,49,57,49 z M56,53h-2v-2h2V53z"
        />
        <path
          fill="#394240"
          d="M7,55h4c0.553,0,1-0.447,1-1v-4c0-0.553-0.447-1-1-1H7c-0.553,0-1,0.447-1,1v4C6,54.553,6.447,55,7,55z M8,51h2v2H8V51z"
        />
        <g opacity="0.15">
          <path d="M2,15v46c0,1.104,0.896,2,2,2h12V13H4C2.896,13,2,13.896,2,15z" />
          <path d="M60,23H48v40h12c1.104,0,2-0.896,2-2V25C62,23.896,61.104,23,60,23z" />
        </g>
        <rect x="29" y="55" fill="#F76D57" width="6" height="8.002" />
        <g>
          <rect x="8" y="21" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="8" y="31" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="8" y="41" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="8" y="51" fill="#506C7F" width="2.001" height="2.002" />
          <rect x="26" y="11" fill="#506C7F" width="2.001" height="2.002" />
          <rect x="26" y="21" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="26" y="31" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="26" y="41" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="36" y="11" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="36" y="21" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="36" y="31" fill="#506C7F" width="2.001" height="2.002" />
          <rect x="36" y="41" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="54" y="31" fill="#F9EBB2" width="2.001" height="2.002" />
          <rect x="54" y="41" fill="#506C7F" width="2.001" height="2.002" />
          <rect x="54" y="51" fill="#F9EBB2" width="2.001" height="2.002" />
        </g>
      </g>
    </g>
  </svg>
);

export const OthersIcon = ({
  size = 28,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    fill="currentColor"
    width={width || size}
    height={width || size}
    viewBox="0 0 1000 1000"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path d="M313 333H153q-8 0-14-6t-6-14V153q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zm265 0H418q-8 0-14-6t-6-14V153q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zm265 0H683q-8 0-14-6t-6-14V153q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zM313 598H153q-8 0-14-6t-6-14V418q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zm265 0H418q-8 0-14-6t-6-14V418q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zm265 0H683q-8 0-14-6t-6-14V418q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zM313 863H153q-8 0-14-6t-6-14V683q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zm265 0H418q-8 0-14-6t-6-14V683q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6zm265 0H683q-8 0-14-6t-6-14V683q0-8 6-14t14-6h160q8 0 14 6t6 14v160q0 8-6 14t-14 6z" />
    </g>
  </svg>
);

export const CoinIcon = ({
  size = 28,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    height={height || size}
    width={width || size}
    version="1.1"
    id="_x32_"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512 512"
    fill="none"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <g>
        <path
          fill="#F9CB43"
          d="M326.27,86.016l57.667,15.304c1.504,0.307,2.726,1.536,3.072,3.049l15.203,57.667 c0.41,1.693,1.899,2.883,3.631,2.883c1.749,0,3.222-1.19,3.615-2.883l15.226-57.667c0.323-1.512,1.552-2.742,3.056-3.049 l57.675-15.304c1.686-0.316,2.884-1.82,2.884-3.553c0-1.749-1.198-3.222-2.884-3.56L427.74,63.606 c-1.504-0.322-2.733-1.536-3.056-3.048L409.458,2.891C409.064,1.213,407.591,0,405.843,0c-1.732,0-3.222,1.213-3.631,2.891 l-15.203,57.667c-0.346,1.512-1.567,2.726-3.072,3.048L326.27,78.903c-1.702,0.339-2.883,1.812-2.883,3.56 C323.387,84.196,324.569,85.701,326.27,86.016z"
        />
        <path
          fill="#F9D689"
          d="M385.253,326.53l-26.176-6.939c-0.685-0.158-1.244-0.694-1.394-1.386l-6.9-26.175 c-0.181-0.764-0.851-1.308-1.646-1.308c-0.78,0-1.457,0.544-1.646,1.308l-6.9,26.175c-0.158,0.693-0.709,1.228-1.394,1.386 l-26.167,6.939c-0.772,0.15-1.316,0.828-1.316,1.615c0,0.788,0.544,1.466,1.316,1.623l26.167,6.932 c0.685,0.141,1.236,0.709,1.394,1.402l6.9,26.159c0.189,0.764,0.866,1.308,1.646,1.308c0.796,0,1.465-0.544,1.646-1.308l6.9-26.159 c0.15-0.693,0.709-1.261,1.394-1.402l26.176-6.932c0.756-0.157,1.308-0.835,1.308-1.623 C386.56,327.357,386.009,326.68,385.253,326.53z"
        />
        <path
          fill="#F9D689"
          d="M37.841,140.075l41.204,10.917c1.086,0.229,1.946,1.104,2.205,2.19l10.854,41.204 c0.299,1.221,1.363,2.064,2.6,2.064c1.244,0,2.3-0.843,2.592-2.064l10.862-41.204c0.229-1.086,1.119-1.961,2.198-2.19 l41.212-10.917c1.205-0.252,2.063-1.339,2.063-2.568c0-1.229-0.858-2.284-2.063-2.536l-41.212-10.934 c-1.079-0.236-1.969-1.086-2.198-2.174L97.296,80.636c-0.292-1.198-1.347-2.048-2.592-2.048c-1.236,0-2.3,0.85-2.6,2.048 l-10.854,41.228c-0.26,1.087-1.119,1.937-2.205,2.174l-41.204,10.934c-1.229,0.252-2.072,1.307-2.072,2.536 C35.769,138.736,36.612,139.824,37.841,140.075z"
        />
        <path
          fill="#F7C566"
          d="M396.595,276.897c-7.877-9.216-19.133-17.392-33.012-24.245h0.016c-27.68-13.706-65.638-22.094-107.583-22.118 l-0.118,0.007v-0.007c-27.546,0.007-53.398,3.623-75.745,9.988c-22.347,6.365-41.156,15.439-54.705,26.467l0.023-0.016 c-9.05,7.357-15.706,15.565-19.4,24.49c-2.112,5.065-3.222,10.405-3.214,15.816c0,4.451,3.608,8.066,8.066,8.066 c4.459,0,8.066-3.615,8.066-8.066c0.008-3.253,0.623-6.348,1.985-9.657c2.355-5.734,7.168-12.051,14.659-18.117l0.023-0.016 c11.154-9.137,28.184-17.581,48.916-23.449c20.732-5.916,45.198-9.374,71.326-9.366h0.425c39.629,0.008,75.429,8.145,100.1,20.433 l0.024,0.008c12.382,6.097,21.937,13.264,27.884,20.259c2.994,3.506,5.12,6.948,6.483,10.248c1.702,4.12,6.412,6.074,10.532,4.372 c4.12-1.701,6.073-6.412,4.372-10.531C403.613,286.373,400.542,281.497,396.595,276.897z"
        />
        <path
          fill="#F7C566"
          d="M485.912,271.966c-3.403-8.208-8.058-15.84-13.675-22.89c-5.616-7.042-12.185-13.517-19.496-19.472 l-0.448-0.362l0.433,0.362c-21.961-17.849-50.862-31.492-84.338-41.07h-0.008c-33.508-9.523-71.735-14.84-112.38-14.84 c-30.972,0-60.527,3.088-87.67,8.767c-27.144,5.68-51.877,13.95-73.208,24.489l-0.016,0.008 c-21.307,10.571-39.424,23.442-52.87,39.14l0.244-0.3l-0.252,0.3c-6.711,7.862-12.256,16.487-16.14,25.868 c-3.891,9.389-6.057,19.511-6.05,29.932v81.873c-0.007,10.43,2.159,20.559,6.05,29.948c3.403,8.208,8.059,15.832,13.675,22.882 c5.624,7.05,12.185,13.517,19.494,19.472l0.016,0.008c21.961,17.833,50.853,31.507,84.33,41.069l0.18,0.04 C177.255,506.683,215.418,511.992,256,512h0.008c30.964-0.008,60.518-3.096,87.662-8.775c27.144-5.68,51.877-13.958,73.208-24.49 l0.015-0.015c21.307-10.572,39.416-23.45,52.87-39.133l0.008-0.015c6.711-7.846,12.248-16.462,16.132-25.852l-0.244,0.575 l0.252-0.575c3.883-9.39,6.05-19.519,6.05-29.948v-81.873C491.961,291.477,489.795,281.355,485.912,271.966z M72.838,426.409 c-2.481-2.339-4.797-4.718-6.86-7.121c-4.892-5.734-8.563-11.602-11.013-17.51c-2.45-5.923-3.678-11.886-3.678-18.006v-37.116 c4.781,5.9,10.429,11.469,16.817,16.684c1.537,1.236,3.112,2.458,4.734,3.671V426.409z M118.083,454.932 c-3.111-1.355-6.175-2.742-9.114-4.206c-6.845-3.387-13.147-7.003-18.896-10.775v-61.676c8.578,4.923,17.951,9.436,28.01,13.501 V454.932z M174.111,472.505c-14.194-3.002-27.514-6.759-39.794-11.146h1.016v-63.29c3.946,1.307,7.995,2.544,12.122,3.726 c8.539,2.426,17.456,4.552,26.656,6.404V472.505z M247.375,480.657c-19.519-0.394-38.282-2.174-56.021-5.041v-64.401 c17.841,2.756,36.604,4.371,56.021,4.75V480.657z M322.804,475.23c-18.369,3.064-37.856,4.986-58.179,5.38v-64.607 c20.196-0.394,39.708-2.15,58.179-5.081V475.23z M378.833,460.965c-6.081,2.206-12.414,4.254-19.008,6.128 c-6.364,1.812-12.996,3.426-19.786,4.907v-64.228c13.651-2.82,26.632-6.333,38.794-10.437V460.965z M424.085,438.484 c-8.16,5.593-17.55,10.792-28.018,15.517v-63.125c5.041-2.088,9.933-4.278,14.604-6.585c4.671-2.324,9.137-4.758,13.414-7.279 V438.484z M460.713,383.771c0,6.12-1.228,12.084-3.686,18.006c-3.151,7.656-8.436,15.274-15.707,22.576v-58.242h-1.024 c7.152-5.459,13.493-11.28,18.826-17.51c0.543-0.638,1.063-1.284,1.59-1.93V383.771z M457.027,319.89 c-4.253,10.334-12.367,20.59-24.024,30.082c-17.448,14.241-42.756,26.593-73.177,35.233 c-30.412,8.665-65.922,13.667-103.825,13.667c-57.746,0.024-109.978-11.658-147.031-30.035 c-18.526-9.16-33.209-19.967-42.992-31.444c-4.892-5.727-8.563-11.595-11.013-17.502c-2.45-5.924-3.678-11.879-3.678-17.991 s1.228-12.067,3.678-17.991c4.27-10.342,12.367-20.598,24.033-30.074c17.456-14.25,42.756-26.608,73.177-35.249 c30.413-8.656,65.93-13.651,103.826-13.651c57.746-0.032,109.969,11.658,147.023,30.027c18.534,9.153,33.224,19.976,42.992,31.436 c4.899,5.727,8.57,11.595,11.012,17.51c2.458,5.924,3.686,11.878,3.686,17.991S459.485,313.966,457.027,319.89z"
        />
      </g>
    </g>
  </svg>
);

export const BillIcon = ({
  size = 28,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    fill="#000000"
    height={height || size}
    width={width || size}
    viewBox="0 0 32 32"
    fillRule="evenodd"
    clipRule="evenodd"
    strokeLinejoin="round"
    strokeMiterlimit={2}
    version="1.1"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <g transform="matrix(1,0,0,1,-144,-192)">
        <g transform="matrix(1,0,0,1,-148,96)">
          <path
            d="M313,101C313,100.47 312.789,99.961 312.414,99.586C312.039,99.211 311.53,99 311,99C307.537,99 300.463,99 297,99C296.47,99 295.961,99.211 295.586,99.586C295.211,99.961 295,100.47 295,101C295,105.843 295,118.157 295,123C295,123.53 295.211,124.039 295.586,124.414C295.961,124.789 296.47,125 297,125C301.69,125 313,125 313,125L313,101Z"
            fill="#90e0ef"
          />
        </g>
        <g transform="matrix(1,0,0,1,-2,0)">
          <path
            d="M174,207C174,205.895 173.105,205 172,205C169.931,205 167,205 167,205L167,221L168,221C171.314,221 174,218.314 174,215C174,212.178 174,209.001 174,207Z"
            fill="#90e0ef"
          />
        </g>
        <path
          d="M149,222L166,222C169.866,222 173,218.866 173,215L173,207C173,205.343 171.657,204 170,204L166,204L166,197C166,196.204 165.684,195.441 165.121,194.879C164.559,194.316 163.796,194 163,194C159.537,194 152.463,194 149,194C148.204,194 147.441,194.316 146.879,194.879C146.316,195.441 146,196.204 146,197L146,219C146,219.796 146.316,220.559 146.879,221.121C147.441,221.684 148.204,222 149,222ZM164,197L164,220C164,220 149,220 149,220C148.735,220 148.48,219.895 148.293,219.707C148.105,219.52 148,219.265 148,219C148,219 148,197 148,197C148,196.735 148.105,196.48 148.293,196.293C148.48,196.105 148.735,196 149,196L163,196C163.265,196 163.52,196.105 163.707,196.293C163.895,196.48 164,196.735 164,197ZM166,206L170,206C170.552,206 171,206.448 171,207L171,215C171,217.761 168.761,220 166,220L166,220L166,206ZM151,217L152,217C152.552,217 153,216.552 153,216C153,215.448 152.552,215 152,215L151,215C150.448,215 150,215.448 150,216C150,216.552 150.448,217 151,217ZM156,217L161,217C161.552,217 162,216.552 162,216C162,215.448 161.552,215 161,215L156,215C155.448,215 155,215.448 155,216C155,216.552 155.448,217 156,217ZM151,213L152,213C152.552,213 153,212.552 153,212C153,211.448 152.552,211 152,211L151,211C150.448,211 150,211.448 150,212C150,212.552 150.448,213 151,213ZM156,213L161,213C161.552,213 162,212.552 162,212C162,211.448 161.552,211 161,211L156,211C155.448,211 155,211.448 155,212C155,212.552 155.448,213 156,213ZM151,209L152,209C152.552,209 153,208.552 153,208C153,207.448 152.552,207 152,207L151,207C150.448,207 150,207.448 150,208C150,208.552 150.448,209 151,209ZM156,209L161,209C161.552,209 162,208.552 162,208C162,207.448 161.552,207 161,207L156,207C155.448,207 155,207.448 155,208C155,208.552 155.448,209 156,209ZM151,205L152,205C152.552,205 153,204.552 153,204C153,203.448 152.552,203 152,203L151,203C150.448,203 150,203.448 150,204C150,204.552 150.448,205 151,205ZM156,205L161,205C161.552,205 162,204.552 162,204C162,203.448 161.552,203 161,203L156,203C155.448,203 155,203.448 155,204C155,204.552 155.448,205 156,205ZM151,201L152,201C152.552,201 153,200.552 153,200C153,199.448 152.552,199 152,199L151,199C150.448,199 150,199.448 150,200C150,200.552 150.448,201 151,201ZM156,201L161,201C161.552,201 162,200.552 162,200C162,199.448 161.552,199 161,199L156,199C155.448,199 155,199.448 155,200C155,200.552 155.448,201 156,201Z"
          fill="#1990a7"
        />
      </g>
    </g>
  </svg>
);

export const FoodIcon = ({
  size = 28,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    fill="#000000"
    height={height || size}
    id="Layer_1"
    version="1.1"
    viewBox="0 0 511.986 511.986"
    width={width || size}
    xmlSpace="preserve"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        d="M184.135,157.173c-2.344,0-4.703-0.766-6.672-2.344c-4.594-3.695-5.32-10.406-1.633-15 c15.547-19.367,49.694-17.031,53.538-16.703c5.867,0.508,10.218,5.672,9.71,11.539c-0.5,5.867-5.664,10.21-11.523,9.718 c-6.836-0.555-27.914-0.133-35.093,8.805C190.353,155.814,187.26,157.173,184.135,157.173z"
        fill="#434A54"
      />
      <path
        d="M186.588,144.064c-20.75-53.303-71.444-52.842-107.654-16.632 c-36.187,36.187-34.031,102.38,6.539,117.747c15.367,40.593,81.561,42.734,117.747,6.539 C239.43,215.508,239.891,164.814,186.588,144.064z"
        fill="#ED5564"
      />
      <path
        d="M346.598,76.12c-3.141,0-6.249-1.383-8.358-4.039c-0.672-0.844-6.609-8.586-9.141-20.679 c-2.375-11.296-2.188-28.671,12.156-47.257c3.608-4.656,10.296-5.523,14.968-1.921c4.656,3.601,5.516,10.296,1.922,14.96 c-7.531,9.75-10.281,19.789-8.172,29.843c1.484,7.07,4.953,11.757,4.984,11.796c3.656,4.617,2.875,11.328-1.734,14.992 C351.255,75.363,348.927,76.12,346.598,76.12z"
        fill="#A0D468"
      />
      <path
        d="M356.63,227.281l38.281-12.656l40.186-13.281c47.875-15.828,14.203-176.191-24.952-163.246 L279.382,81.331C240.071,94.323,308.616,243.156,356.63,227.281z"
        fill="#E9573F"
      />
      <path
        d="M422.613,105.08c-7.781-23.562-16.905-62.943-39.999-62.943c-2.375,0-4.875,0.414-7.547,1.297 l-64.498,21.319c-28.781,9.516-12.812,43.155-1.344,77.827c10.781,32.593,40.842,84.818,62.998,84.818 c1.422,0,2.797-0.211,4.141-0.656l42.421-14.023C441.159,205.321,431.206,131.049,422.613,105.08z"
        fill="#FC6E51"
      />
      <path
        d="M437.315,298.679H74.66c-5.891,0-10.664,4.766-10.664,10.655v32 c0,70.982,38.921,135.902,101.576,169.402c1.547,0.828,3.273,1.25,5.031,1.25H341.38c1.749,0,3.483-0.422,5.03-1.25 c62.655-33.5,101.576-98.42,101.576-169.402v-32C447.987,303.444,443.206,298.679,437.315,298.679z"
        fill="#A0D468"
      />
      <path
        opacity="0.2"
        fill="#ffffff"
        enableBackground="new"
        d="M437.315,298.679h-21.327c5.891,0,10.672,4.766,10.672,10.655 v32c0,70.982-38.937,135.902-101.592,169.402c-1.547,0.828-3.266,1.25-5.03,1.25h21.343c1.749,0,3.483-0.422,5.03-1.25 c62.655-33.5,101.576-98.42,101.576-169.402v-32C447.987,303.444,443.206,298.679,437.315,298.679z"
      />
    </g>
  </svg>
);

export const FileIcon = ({
  size = 20,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    width={width || size}
    height={height || size}
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        d="M19 9V17.8C19 18.9201 19 19.4802 18.782 19.908C18.5903 20.2843 18.2843 20.5903 17.908 20.782C17.4802 21 16.9201 21 15.8 21H8.2C7.07989 21 6.51984 21 6.09202 20.782C5.71569 20.5903 5.40973 20.2843 5.21799 19.908C5 19.4802 5 18.9201 5 17.8V6.2C5 5.07989 5 4.51984 5.21799 4.09202C5.40973 3.71569 5.71569 3.40973 6.09202 3.21799C6.51984 3 7.0799 3 8.2 3H13M19 9L13 3M19 9H14C13.4477 9 13 8.55228 13 8V3"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </g>
  </svg>
);

export const HeartFilledIcon = ({
  size = 24,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    aria-hidden="true"
    focusable="false"
    height={size || height}
    role="presentation"
    viewBox="0 0 24 24"
    width={size || width}
    {...props}
  >
    <path
      d="M12.62 20.81c-.34.12-.9.12-1.24 0C8.48 19.82 2 15.69 2 8.69 2 5.6 4.49 3.1 7.56 3.1c1.82 0 3.43.88 4.44 2.24a5.53 5.53 0 0 1 4.44-2.24C19.51 3.1 22 5.6 22 8.69c0 7-6.48 11.13-9.38 12.12Z"
      fill="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth={1.5}
    />
  </svg>
);

export const CheckIcon = ({
  size = 24,
  width,
  height,
  fill = "white",
  strokeWidth = 3,
  ...props
}: IconSvgProps) => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    width={width || size}
    height={height || size}
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        d="M4 12.6111L8.92308 17.5L20 6.5"
        stroke={fill}
        strokeWidth={strokeWidth}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </g>
  </svg>
);

export const InvalidReportIcon = ({
  size = 24,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    height={size || height}
    width={size || width}
    version="1.1"
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512 512"
    fill="#000000"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        fill="#FF9900"
        d="M507.494,426.066L282.864,53.536c-5.677-9.415-15.87-15.172-26.865-15.172 c-10.994,0-21.188,5.756-26.865,15.172L4.506,426.066c-5.842,9.689-6.015,21.774-0.451,31.625 c5.564,9.852,16.001,15.944,27.315,15.944h449.259c11.314,0,21.751-6.093,27.315-15.944 C513.509,447.839,513.336,435.755,507.494,426.066z"
      />
      <path
        fill="#FFDC35"
        d="M255.999,38.365c-10.994,0-21.188,5.756-26.865,15.172L4.506,426.066 c-5.842,9.689-6.015,21.774-0.451,31.625c5.564,9.852,16.001,15.944,27.315,15.944h224.629L255.999,38.365L255.999,38.365z"
      />
      <path
        fill="#F20013"
        d="M445.326,432.791H67.108c-3.591,0-6.911-1.909-8.718-5.012c-1.807-3.104-1.827-6.934-0.055-10.056 L247.23,85.028c1.792-3.155,5.139-5.106,8.767-5.107c0.001,0,0.003,0,0.004,0c3.626,0,6.974,1.946,8.767,5.099l189.324,332.694 c1.777,3.123,1.759,6.955-0.047,10.061S448.918,432.791,445.326,432.791z M84.436,412.616h343.543L256.013,110.423L84.436,412.616z"
      />
      <path
        fill="#FF4B00"
        d="M256.332,412.616H84.436l171.576-302.192l-0.01-30.501h-0.005c-3.628,0.001-6.976,1.951-8.767,5.107 L58.336,417.722c-1.773,3.123-1.752,6.953,0.055,10.056c1.807,3.104,5.127,5.012,8.718,5.012h189.224v-20.175H256.332z"
      />
      <path
        fill="#533F29"
        d="M279.364,376.883c0,12.344-10.537,23.182-22.88,23.182c-13.246,0-23.182-10.838-23.182-23.182 c0-12.644,9.935-23.182,23.182-23.182C268.826,353.701,279.364,364.238,279.364,376.883z M273.644,319.681 c0,9.333-10.236,13.246-17.462,13.246c-9.634,0-17.762-3.914-17.762-13.246c0-35.826-4.214-87.308-4.214-123.134 c0-11.741,9.634-18.365,21.977-18.365c11.741,0,21.677,6.623,21.677,18.365C277.858,232.373,273.644,283.855,273.644,319.681z"
      />
    </g>
  </svg>
);

export const ValidReportIcon = ({
  size = 24,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    viewBox="0 0 36 36"
    xmlns="http://www.w3.org/2000/svg"
    aria-hidden="true"
    role="img"
    height={height || size}
    width={width || size}
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <path
        fill="#EF9645"
        d="M23.216 20.937l-1.721-6.86l-3.947-8.816a2.518 2.518 0 0 1 1.44-3.257a2.516 2.516 0 0 1 3.258 1.44l6.203 15.769l-5.233 1.724z"
      />
      <path
        fill="#FFDC5D"
        d="M31.565 18.449c-.488-2.581-1.988-6.523-1.988-6.523L23.79 1.437a2.518 2.518 0 1 0-4.498 2.268l5.33 9.547c.013.022 1.413 5.491 1.413 5.491c-1.078-.995-2.607-2.359-4.015-3.618c-3.098-2.772-4.936-3.811-4.936-3.811c-.71-.443-1.179-.506-2.132-.059L9.08 13.823c-.157.078-.29.188-.395.329l-2.313 3.086a2.517 2.517 0 0 0 .315 3.547a2.521 2.521 0 0 0 3.548-.314c.048-.058 1.78-2.56 1.936-2.64c1.037-.533 2.965-1.447 3.808-1.42c.897.029 6.281 5.957 6.281 5.957c.206.259.23.618.06.902l-2.915 5.228a.79.79 0 0 1-.33.303l-2.674 1.5a.794.794 0 0 1-.496.067l-5.27-2.272a1.125 1.125 0 0 1-.592-.583a12.297 12.297 0 0 1-.899-3.639a2.518 2.518 0 0 0-2.634-2.398a2.514 2.514 0 0 0-2.398 2.633c.013.263.351 5.64 4.727 9.292c2.528 2.108 5.654 2.924 9.649 2.387c4.612-.619 7.469-1.233 11.506-9.558c1.117-2.305 1.903-6.024 1.571-7.781z"
      />
    </g>
  </svg>
);

export const SpamIcon = ({
  size = 24,
  width,
  height,
  ...props
}: IconSvgProps) => (
  <svg
    height={height || size}
    width={width || size}
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 370 370"
    fill="#000000"
    {...props}
  >
    <g id="SVGRepo_bgCarrier" strokeWidth="0" />
    <g
      id="SVGRepo_tracerCarrier"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <g id="SVGRepo_iconCarrier">
      <g id="XMLID_7_">
        <rect
          id="XMLID_484_"
          x="5"
          y="140"
          fill="#CCF7F9"
          width="360"
          height="230"
        />
        <rect
          id="XMLID_486_"
          x="185"
          y="140"
          fill="#00D7DF"
          width="180"
          height="230"
        />
        <polygon
          id="XMLID_487_"
          fill="#00A1A7"
          points="185,280 5,140 185,120 365,140 "
        ></polygon>
        <polygon
          id="XMLID_488_"
          fill="#CCF7F9"
          points="185,0 5,140 365,140 "
        ></polygon>
        <polygon
          id="XMLID_489_"
          fill="#00D7DF"
          points="185,0 185,140 365,140 "
        ></polygon>
        <polygon
          id="XMLID_490_"
          fill="#006C70"
          points="185,280 185,140 365,140 "
        ></polygon>
        <g id="XMLID_47_">
          <path
            id="XMLID_493_"
            fill="#FFFFFF"
            d="M123.299,314.392c-0.135-0.179-0.605-0.515-1.412-1.009 c-0.807-0.493-1.816-1.009-3.025-1.547c-1.211-0.538-2.533-1.009-3.969-1.412c-1.436-0.403-2.869-0.605-4.305-0.605 c-3.945,0-5.918,1.323-5.918,3.968c0,0.808,0.213,1.479,0.641,2.018c0.424,0.538,1.053,1.02,1.883,1.446 c0.828,0.427,1.871,0.818,3.127,1.177c1.254,0.359,2.711,0.763,4.371,1.21c2.287,0.629,4.348,1.312,6.188,2.052 c1.838,0.739,3.395,1.659,4.674,2.757c1.277,1.1,2.262,2.433,2.959,4.002c0.693,1.569,1.041,3.452,1.041,5.648 c0,2.69-0.504,4.966-1.512,6.826c-1.01,1.86-2.344,3.362-4.002,4.506c-1.66,1.143-3.564,1.973-5.717,2.488 c-2.152,0.515-4.371,0.774-6.658,0.774c-1.748,0-3.541-0.135-5.379-0.404c-1.84-0.269-3.631-0.66-5.381-1.177 c-1.748-0.515-3.441-1.131-5.076-1.849c-1.639-0.717-3.15-1.547-4.539-2.488l4.842-9.617c0.178,0.225,0.762,0.65,1.748,1.278 c0.984,0.628,2.207,1.255,3.664,1.883s3.082,1.189,4.877,1.681c1.793,0.494,3.607,0.74,5.447,0.74c3.9,0,5.85-1.188,5.85-3.564 c0-0.897-0.291-1.636-0.873-2.219c-0.584-0.583-1.391-1.11-2.422-1.581s-2.252-0.907-3.664-1.312 c-1.412-0.403-2.949-0.85-4.607-1.345c-2.197-0.673-4.102-1.4-5.717-2.185c-1.613-0.784-2.947-1.691-4-2.724 c-1.055-1.031-1.84-2.219-2.355-3.564c-0.516-1.345-0.773-2.914-0.773-4.707c0-2.511,0.473-4.729,1.414-6.658 c0.941-1.927,2.219-3.553,3.832-4.875c1.615-1.322,3.486-2.32,5.615-2.993c2.129-0.672,4.383-1.009,6.76-1.009 c1.658,0,3.295,0.158,4.908,0.471c1.615,0.314,3.16,0.718,4.641,1.211c1.479,0.494,2.857,1.054,4.137,1.681 c1.277,0.629,2.453,1.256,3.529,1.884L123.299,314.392z"
          />
          <path
            id="XMLID_495_"
            fill="#FFFFFF"
            d="M139.371,348.151v-47.748h20.377c2.287,0,4.381,0.471,6.287,1.413 c1.906,0.941,3.541,2.175,4.91,3.698c1.367,1.525,2.443,3.251,3.229,5.179c0.783,1.928,1.176,3.878,1.176,5.851 c0,2.107-0.369,4.125-1.109,6.052c-0.74,1.929-1.771,3.655-3.094,5.179c-1.322,1.524-2.926,2.735-4.809,3.632 c-1.883,0.897-3.967,1.345-6.254,1.345H150.4v15.4L139.371,348.151L139.371,348.151z M150.4,323.134h9.012 c1.299,0,2.42-0.56,3.361-1.681c0.941-1.12,1.414-2.758,1.414-4.909c0-1.12-0.146-2.085-0.438-2.892 c-0.293-0.808-0.686-1.479-1.178-2.018c-0.494-0.538-1.053-0.931-1.682-1.177c-0.627-0.246-1.256-0.37-1.883-0.37H150.4V323.134z"
          />
          <path
            id="XMLID_498_"
            fill="#FFFFFF"
            d="M194.582,300.403h9.953l17.418,47.748h-11.299l-3.699-10.693h-14.861 l-3.631,10.693h-11.299L194.582,300.403z M205.141,329.859l-5.582-16.88l-5.717,16.88H205.141z"
          />
          <path
            id="XMLID_502_"
            fill="#FFFFFF"
            d="M267.684,348.151V319.57l-10.357,20.78h-5.918l-10.357-20.78v28.581h-11.027 v-47.748h11.971l12.373,24.95l12.441-24.95h11.902v47.748H267.684z"
          />
        </g>
        <g id="XMLID_8_">
          <polygon
            id="XMLID_46_"
            points="225.5,136.706 262.373,99.833 249.016,86.476 225.5,109.992 "
          ></polygon>
          <polygon
            id="XMLID_45_"
            points="225.5,188.887 249.016,212.403 262.373,199.046 225.5,162.173 "
          ></polygon>
          <rect
            id="XMLID_44_"
            x="225.5"
            y="140"
            width="47.648"
            height="18.889"
          />
          <g id="XMLID_40_">
            <polygon
              id="XMLID_505_"
              fill="#006C70"
              points="141,106.492 120.984,86.476 107.627,99.833 141,133.206 "
            ></polygon>
            <polygon
              id="XMLID_506_"
              fill="#006C70"
              points="141,165.673 107.627,199.046 120.984,212.403 141,192.387 "
            ></polygon>
            <rect
              id="XMLID_507_"
              x="96.852"
              y="140"
              fill="#006C70"
              width="44.148"
              height="18.889"
            />
          </g>
          <circle
            id="XMLID_39_"
            fill="#FF9811"
            cx="185"
            cy="86.476"
            r="37.777"
          ></circle>
          <path
            id="XMLID_38_"
            fill="#FF5023"
            d="M185,48.698c20.864,0,37.777,16.915,37.777,37.778 c0,20.864-16.913,37.778-37.777,37.778"
          />
          <circle
            id="XMLID_37_"
            fill="#FFDA44"
            cx="185"
            cy="149.439"
            r="69.26"
          ></circle>
          <path
            id="XMLID_36_"
            fill="#FF9811"
            d="M185,80.181c38.251,0,69.26,31.007,69.26,69.259 c0,38.251-31.009,69.259-69.26,69.259"
          />
          <circle
            id="XMLID_35_"
            fill="#FF9811"
            cx="153.519"
            cy="124.254"
            r="12.593"
          ></circle>
          <circle
            id="XMLID_34_"
            fill="#FF5023"
            cx="216.481"
            cy="124.254"
            r="12.593"
          ></circle>
          <circle
            id="XMLID_33_"
            fill="#FF9811"
            cx="185"
            cy="174.624"
            r="18.889"
          ></circle>
          <path
            id="XMLID_32_"
            fill="#FF5023"
            d="M185,155.736c10.432,0,18.889,8.457,18.889,18.888 c0,10.433-8.457,18.89-18.889,18.89"
          />
        </g>
      </g>
    </g>
  </svg>
);

export const SearchIcon = (props: IconSvgProps) => (
  <svg
    aria-hidden="true"
    fill="none"
    focusable="false"
    height="1em"
    role="presentation"
    viewBox="0 0 24 24"
    width="1em"
    {...props}
  >
    <path
      d="M11.5 21C16.7467 21 21 16.7467 21 11.5C21 6.25329 16.7467 2 11.5 2C6.25329 2 2 6.25329 2 11.5C2 16.7467 6.25329 21 11.5 21Z"
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="2"
    />
    <path
      d="M22 22L20 20"
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="2"
    />
  </svg>
);
